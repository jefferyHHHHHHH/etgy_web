...
- 确定范围：新功能 vs 修改现有功能
- 选择唯一的 `change-id`：使用 kebab-case 命名法，动词开头（`add-`、`update-`、`remove-`、`refactor-`）
- 搭建结构：为每个受影响的功能创建 `proposal.md`、`tasks.md`、`design.md`（仅在需要时）和增量规范
- 编写增量：使用 `## ADDED|MODIFIED|REMOVED|RENAMED Requirements`；每个需求至少包含一个 `#### Scenario:`
- 验证：执行 `openspec validate [change-id] --strict` 并修复问题
...
2. 选择一个唯一的动词开头的 `change-id`，在 `openspec/changes/<id>/` 下创建 `proposal.md`、`tasks.md`、可选的 `design.md` 和规范增量。
...
- 如果功能发生变化，更新 `specs/`
- 对于仅工具变更使用 `openspec archive <change-id> --skip-specs --yes`（始终显式传递 change ID）
...
1. **创建目录：** `changes/[change-id]/`（kebab-case 命名法，动词开头，唯一）
...
- ADDED：引入一个可以作为需求独立存在的新功能或子功能。当变更是正交的（例如，添加"斜杠命令配置"）而不是改变现有需求的语义时，优先使用 ADDED。
- MODIFIED：更改现有需求的行为、范围或验收标准。始终粘贴完整的、更新后的需求内容（标题 + 所有场景）。归档器将用你在此处提供的内容替换整个需求；部分增量会丢失以前的详细信息。
- RENAMED：仅当名称更改时使用。如果同时更改行为，使用 RENAMED（名称）加上 MODIFIED（内容）引用新名称。

常见陷阱：使用 MODIFIED 添加新关注点但不包含以前的文本。这会在归档时导致详细信息丢失。如果你没有明确更改现有需求，请在 ADDED 下添加新需求。
